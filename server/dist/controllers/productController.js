...
exports.submitBid = exports.getSellerProducts = exports.createCategory = exports.getCategories = exports.deleteProduct = exports.updateProduct = exports.createProduct = exports.getProductBySlug = exports.getProducts = void 0;
...
exports.getProducts = (0, errors_1.catchAsync)(async (req, res, next) => {
    ...
    if (category) {
        where.category = {
            OR: [
                { slug: category },
                { parent: { slug: category } } // Handles parent/subcategories
            ]
        };
    }
    ...
    if (rating) {
        where.ratings = { gte: parseFloat(rating) };
    }
    if (inStock === 'true') {
        where.variants = {
            some: {
...
exports.getProductBySlug = (0, errors_1.catchAsync)(async (req, res, next) => {
    ...
    if (product.seller) {
        const sellerProducts = await db_1.default.product.findMany({
            where: { sellerId: product.seller.id },
            select: { id: true }
        });
        const productIds = sellerProducts.map((p) => p.id);
        const ratingsGroup = await db_1.default.review.aggregate({
            where: {
    ...
    const enhancedSeller = product.seller
        ...
        : null;
    const bids = await db_1.default.auctionBid.findMany({
...
exports.createProduct = (0, errors_1.catchAsync)(async (req, res, next) => {
    const { name, description, price, discountPrice, categoryId, images, variants, brand, color, badge, specifications, features, flashSaleEnd, flashSaleDiscount, auctionEnd, minBid } = req.body;
    if (!req.user)
    ...
    if (!seller) {
        return next(new errors_1.AppError('Seller profile not found. Cannot create product.', 404));
    }
    ...
    const newProduct = await db_1.default.$transaction(async (tx) => {
        ...
        if (!categoryExists) {
            throw new errors_1.AppError('Specified category does not exist', 400);
        }
    ...
    const completeProduct = await db_1.default.product.findUnique({
...
exports.updateProduct = (0, errors_1.catchAsync)(async (req, res, next) => {
    const { id } = req.params;
    const { name, description, price, discountPrice, categoryId, images, variants, brand, color, badge, specifications, features, flashSaleEnd, flashSaleDiscount, auctionEnd, minBid } = req.body;
    if (!req.user)
        return next(new errors_1.AppError('Not authenticated', 401));
    ...
    if (existingProduct.sellerId !== seller.id && req.user.role !== 'ADMIN') {
        return next(new errors_1.AppError('You do not have permission to update this product', 403));
    }
    const updatedProduct = await db_1.default.$transaction(async (tx) => {
        ...
        if (flashSaleEnd !== undefined)
            data.flashSaleEnd = flashSaleEnd ? new Date(flashSaleEnd) : null;
        if (flashSaleDiscount !== undefined)
            data.flashSaleDiscount = flashSaleDiscount ? parseFloat(flashSaleDiscount) : null;
        if (auctionEnd !== undefined)
            data.auctionEnd = auctionEnd ? new Date(auctionEnd) : null;
        if (minBid !== undefined)
            data.minBid = minBid ? parseFloat(minBid) : null;
...
exports.deleteProduct = (0, errors_1.catchAsync)(async (req, res, next) => {
    const { id } = req.params;
    if (!req.user)
        return next(new errors_1.AppError('Not authenticated', 401));
    const existingProduct = await db_1.default.product.findUnique({ where: { id } });
    ...
    if (req.user.role !== 'ADMIN') {
        ...
        if (!seller || existingProduct.sellerId !== seller.id) {
            return next(new errors_1.AppError('You do not have permission to delete this product', 403));
        }
    }
...
// Categories logic
exports.getCategories = (0, errors_1.catchAsync)(async (req, res, next) => {
...
exports.createCategory = (0, errors_1.catchAsync)(async (req, res, next) => {
...
exports.getSellerProducts = (0, errors_1.catchAsync)(async (req, res, next) => {
    if (!req.user)
        return next(new errors_1.AppError('Not authenticated', 401));
    const seller = await db_1.default.seller.findUnique({ where: { userId: req.user.id } });
    if (!seller) {
        return next(new errors_1.AppError('Seller profile not found. Cannot retrieve products.', 404));
    }
...
exports.submitBid = (0, errors_1.catchAsync)(async (req, res, next) => {
    ...
    if (isNaN(amount) || amount <= 0) {
        return next(new errors_1.AppError('Please enter a valid bid amount', 400));
    }
    const product = await db_1.default.product.findUnique({
        where: { id: productId },
        include: {
    ...
    if (!product.auctionEnd || !product.minBid) {
        return next(new errors_1.AppError('This product is not set up for auction', 400));
    }
    if (new Date(product.auctionEnd) < new Date()) {
        return next(new errors_1.AppError('The auction for this product has already ended', 400));
    }
    if (amount < product.minBid) {
        return next(new errors_1.AppError(`Bid amount must be at least $${product.minBid}`, 400));
    }
    const highestBid = product.bids[0];
    if (highestBid && amount <= highestBid.bidAmount) {
        return next(new errors_1.AppError(`Bid amount must be higher than current highest bid of $${highestBid.bidAmount}`, 400));
    }
    // Create new bid
    const bid = await db_1.default.auctionBid.create({
        data: {
            productId,
            userId: req.user.id,
            bidAmount: amount
