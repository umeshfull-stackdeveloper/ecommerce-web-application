...
class EmailService {
    ...
    async sendEmail(to, subject, html) {
        ...
        try {
            ...
            if (this.testAccount) {
                console.log(`✉️ Preview sent email here: ${nodemailer_1.default.getTestMessageUrl(info)}`);
            }
        }
        catch (error) {
    ...
    async sendOrderConfirmationEmail(to, name, orderId, totalAmount) {
        const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 8px;">
    ...
    async sendShippingUpdateEmail(to, name, orderId, status) {
        const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 8px;">
